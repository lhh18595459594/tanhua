...
public class HuanXinTemplate {
     ...
     * @param validateCode
     */
    public void sendValidateCode(String phone, String validateCode){
        String url = props.getHuanXinUrl(); // REST API访问地址：a31.easemob.com
        ///{orgname}/{appname}/sms/send
        // 发验证短信的完整api路径
        url+="/sms/send";
        ...
        requestBody.put("mobiles",new String[]{phone});
        requestBody.put("tid", "183");

        Map<String, Object> msgParam = new HashMap<String, Object>();
        msgParam.put("p1", validateCode);

        requestBody.put("tmap", msgParam);
        ...
        try {
            ...
            log.info("发送验证码*****url:{}******sms/send:{}:{}******statusCode{}*****",url,phone,validateCode,statusCode);
            if(200 != statusCode){
    ...
    public void registerBatch(){
        ...
        for (int i = 10; i < 20 ; i++) {
            list.add(new HuanXinUser(i+"",i+"",String.format("淡水维哥_%05d",i)));
        }
        // 请求头信息
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type","application/json");
